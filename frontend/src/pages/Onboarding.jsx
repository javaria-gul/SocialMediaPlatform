...
const Onboarding = () => {
  ...
  // Follow Suggestions Step Component
  const FollowStep = () => {
    const [selectedUsers, setSelectedUsers] = useState(formData.following || []);

    const suggestedUsers = [
      { id: 1, name: 'Dr. Ali Khan', username: 'alikhan', role: 'faculty', avatar: '👨‍🏫' },
      { id: 2, name: 'Prof. Sara Ahmed', username: 'saraahmed', role: 'faculty', avatar: '👩‍🏫' },
      { id: 3, name: 'Ahmed Raza', username: 'ahmedraza', role: 'student', avatar: '👨‍🎓' },
      { id: 4, name: 'Fatima Noor', username: 'fatimanoor', role: 'student', avatar: '👩‍🎓' },
      { id: 5, name: 'Bilal Hussain', username: 'bilalh', role: 'student', avatar: '🧑‍🎓' },
    ];
    ...
    const isMinimumSelected = selectedUsers.length >= 3;
    ...
    return (
      <div className="space-y-6">
        <div className="text-center">
          ...
          <div className={`inline-flex items-center gap-2 px-4 py-2 rounded-full ${
            ...
            <span className="text-sm font-medium">
              {selectedUsers.length}/3 selected
            </span>
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center space-x-3">
                  <div className={`text-2xl p-2 rounded-full ${
                    selectedUsers.includes(user.id) ? 'bg-purple-100' : 'bg-gray-100'
                  }`}>
                    {user.avatar}
                  </div>
                  <div>
        ...
        {!isMinimumSelected && selectedUsers.length > 0 && (
          <motion.p
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            className="text-center text-orange-500 text-sm font-medium"
          >
            ⚠️ Select {3 - selectedUsers.length} more to continue
          </motion.p>
        )}
  ...
  const handleComplete = async () => {
    try {
      ...
      // Complete onboarding and update user data
      const userProfile = {
